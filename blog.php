...
if ($usedRes) {
    while ($row = $usedRes->fetch_assoc()) {
        foreach (array_map('trim', explode(',', $row['category'])) as $c) {
            if ($c && !in_array($c, $usedCats)) $usedCats[] = $c;
        }
    }
}
...
?>
      ...
      <div class="elephant-nav"
        ><a href="index.html">Home</a> | <span>Blog</span></div
      >
    </section>
  ...
  <div class="page-container">
    ...
    <div class="category-chips">
      ...
      <?php foreach ($groupNames as $gName): ?>
        <a class="chip <?= $activeGroup === $gName ? 'active' : '' ?>" href="blog.php?category=<?= urlencode($gName) ?>"><?= htmlspecialchars($gName) ?></a>
        
        <?php if ($activeGroup === $gName && isset($groupedCats[$gName])): ?>
          <?php foreach ($groupedCats[$gName] as $sub): ?>
            <a class="chip <?= $activeSub === $sub ? 'active' : '' ?>" style="<?= $activeSub === $sub ? 'background:var(--accent-lit);color:var(--secondary);border-color:var(--accent-lit);' : 'opacity:0.8; font-size:13px;' ?>" href="blog.php?category=<?= urlencode($gName) ?>&sub=<?= urlencode($sub) ?>">↳ <?= htmlspecialchars($sub) ?></a>
          <?php endforeach; ?>
        <?php endif; ?>
      ...
      <?php foreach ($posts as $blogPost): ?>
        <div class="blog-card">
          <?php if (!empty($blogPost['cover_image'])): ?>
            <img src="<?= htmlspecialchars($blogPost['cover_image']) ?>" alt="<?= htmlspecialchars($blogPost['title']) ?>">
          <?php else: ?>
            <img src="assets/public/placeholder.jpg" alt="Post Image">
          <?php endif; ?>
          <div class="blog-card-body">
            ...
            <p class="excerpt"><?= htmlspecialchars(mb_strimwidth($blogPost['excerpt'] ?? strip_tags($blogPost['body']), 0, 100, '...')) ?></p>
            <a class="read-more" href="post.php?slug=<?= htmlspecialchars($blogPost['slug']) ?>">Read More →</a>
          </div>
        </div>
      <?php endforeach; ?>

      <?php if (empty($posts)): ?>
        <p style="grid-column:1/-1;text-align:center;padding:40px 0;">No posts found<?= $activeGroup !== 'All' ? ' in the "' . htmlspecialchars($activeSub ?: $activeGroup) . '" category' : '' ?>.</p>
      <?php endif; ?>
    </div>
    ...
    <?php
      $paginationParams = '';
      if ($activeGroup !== 'All') $paginationParams .= 'category=' . urlencode($activeGroup) . '&';
      if (!empty($activeSub)) $paginationParams .= 'sub=' . urlencode($activeSub) . '&';
    ?>
    <div class="pagination" style="margin-top:40px;display:flex;justify-content:center;gap:10px;">
      ...
      <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        <a href="blog.php?<?= $paginationParams ?>page=<?= $i ?>" class="chip <?= $i === $page ? 'active' : '' ?>"><?= $i ?></a>
      <?php endfor; ?>
